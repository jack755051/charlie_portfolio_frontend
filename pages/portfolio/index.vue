<template>
  <div class="scrollable-content-wrapper h-screen overflow-hidden relative bg-slate-50">
    ...
    ></div>
    <div
      class="absolute top-[-10%] right-[-5%] h-[500px] w-[500px] rounded-full bg-orange-200/40 blur-[120px] pointer-events-none"
    ></div>
    <div
      class="absolute bottom-[-10%] left-[-10%] h-[400px] w-[400px] rounded-full bg-blue-200/30 blur-[120px] pointer-events-none"
    ></div>
    ...
    >
      <div class="w-full pb-20">
        <ScrollSection ref="portfolioSection" id="section1" :class="['flex-col !justify-start']">
          <template #title>
            <div class="flex w-full py-8 items-center justify-center">
              <div class="relative pb-2">
                ...
                <div
                  class="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-1 bg-gradient-to-r from-orange-500 to-amber-400 rounded-full"
                ></div>
              </div>
            </div>
          </template>
          ...
          <template #content>
            <div
              v-if="mockStore.portfolioPageData.length === 0"
...
const companyProjects = computed(() => {
  return mockStore.portfolioPageData.filter(
    project =>
